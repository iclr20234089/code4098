...
def evaluate_on_environment(
    ...
    def scorer(algo: AlgoProtocol, *args: Any) -> float:
        ...
        for ii in range(n_trials):
            ...
            print( "score:", float(episode_reward))
...
def done_prediction_scorer(
    ...
    for episode in episodes:
        for batch in _make_batches(episode, WINDOW_SIZE, dynamics.n_frames):
            done = dynamics.predict(batch.observations, batch.actions)
            rewards = batch.terminals
            # print(done)
            m=nn.Sigmoid()

            a = m(torch.from_numpy(done))
