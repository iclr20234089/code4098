...
class DynamicsBase(LearnableBase):
    ...
    def predict_gpu(
        ...
        """
        assert self._impl is not None, IMPL_NOT_INITIALIZED_ERROR
        observations, rewards, variances = self._impl.predict_gpu(
            x,
            action,
            indices,
        )
    ...
    def predict_gpu2(
        ...
        """
        assert self._impl is not None, IMPL_NOT_INITIALIZED_ERROR
        observations, rewards, variances = self._impl.predict_gpu2(
            x,
            action,
            indices,
        )
...
class DoneBase(LearnableBase):

    _impl: Optional[DoneImplBase]
